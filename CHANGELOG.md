...
### Changed
- **`shipnode run <cmd>`**: Run commands through the project-specific Node.js runtime configured by `NODE_VERSION`.
- **`shipnode run <cmd> --profile <name>`** and **`--config <file>`**: Documented profile/custom config usage for one-off remote commands.
- **`shipnode run` package binaries**: Repair execute permissions only for package-declared binaries in `node_modules` before running commands, preventing package CLI `Permission denied` errors without broadly chmodding the app tree.

### Documentation
- Updated README, command reference, multi-environment guide, and ShipNode skill references for the new `shipnode run` behavior.
...
### Fixed
- **harden**: Fix SSH lockout when changing port — `SSH_PORT` was updated immediately after writing the config, causing all subsequent `remote_exec` calls (UFW pre-open, further SSH config changes, service restart) to connect on the not-yet-active new port. `SSH_PORT` is now updated only after the service restarts
- **harden**: Filter current port from SSH port selection options to avoid offering the port that's already in use
...
### Fixed
- **harden**: Replaced all `gum` prompts with plain `read`/`prompt_yes_no` — gum was silently auto-skipping all confirmations due to TTY consumption
