...
# Legacy non-interactive init (print mode - outputs to stdout)
cmd_init_legacy_print() {
    cat << 'EOF'
# App type: "backend" or "frontend"
APP_TYPE=backend
...
# Template-based initialization (print mode - outputs to stdout)
cmd_init_template_print() {
    local template_name="$1"
    ...
    if [ "$USE_GUM" = true ]; then
        gum style \
            ...
            --margin "1 0" \
            --padding "1 2" \
            "ShipNode Template Setup (Print Mode)" \
            "" \
            "Template: $template_name" \
            "" \
        ...
        echo -e "${BLUE}║  ShipNode Template Setup           ║${NC}"
        echo -e "${BLUE}║  Template: $(printf '%-24s' "$template_name") ║${NC}"
        echo -e "${BLUE}║  (Print Mode - stdout output)      ║${NC}"
        echo -e "${BLUE}╚════════════════════════════════════╝${NC}"
    fi
    echo ""
    ...
    # Final confirmation
    if [ "$USE_GUM" = true ]; then
        if ! gum confirm "Generate configuration?"; then
            warn "Configuration cancelled"
            return 1
        fi
    else
        if ! prompt_yes_no "Generate configuration?" "y"; then
            warn "Configuration cancelled"
            return 1
        fi
    fi

    emit_config_draft "Generated from template: $template_name"
}
...
cmd_init_interactive() {
    ...
    app_name=$(suggest_config_app_name)
...
# Interactive initialization wizard (print mode - outputs to stdout)
cmd_init_interactive_print() {
    # Welcome banner
    echo ""
    if [ "$USE_GUM" = true ]; then
        gum style \
            ...
            --margin "1 0" \
            --padding "1 2" \
            "ShipNode Interactive Setup (Print Mode)" \
            "" \
            "Configuration will be printed to stdout"
    else
        echo -e "${BLUE}╔════════════════════════════════════╗${NC}"
    ...
    if [ "$USE_GUM" = true ]; then
        if gum confirm "Would you like to use a framework template?"; then
            ...
            if [ -n "$use_template" ]; then
                cmd_init_template_print "$use_template"
                return $?
            fi
        fi
    else
    ...
    emit_config_draft "Generated by interactive wizard"
}
...
cmd_init() {
    ...
    if [ "$list_templates_flag" = true ]; then
        list_templates
    elif [ "$non_interactive_flag" = true ]; then
        ...
        if [ "$print_mode" = true ]; then
            cmd_init_template_print "$template_name"
        else
            cmd_init_template "$template_name"
        fi
